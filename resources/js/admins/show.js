$(function () {
  ...
  $('.basic_reset_btn').on('click', function (e) {
    ...
    Swal.fire({
      ...
      text: langs.reset_question
        .replace(':tenant', tenant)
        .replace(':what', 'Basic認識'),
      icon: 'warning',
  ...
  function addRow(id, login_id, user_name, timestamp, edited_user) {
    ...
    // Create and append the first cell with the checkbox
    var checkboxCell = $('<td></td>')
      .addClass('px-6 py-4')
      .append(
        $('<input>')
          .attr('type', 'checkbox')
          .addClass('rounded border-gray-600 text-blue-600 focus:ring-blue-500')
      )
    ...
    row
      .append(
        $('<td></td>')
          .addClass('p-3 text-sm text-blue-300 cursor-pointer edit_user')
          .text(login_id)
      )
      .attr('data-id', id) // Add data-id attribute to the td
    row.append(
      $('<td></td>')
        .addClass(
          'px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white'
  ...
  $(document).on('click', '.edit_user', function () {
    ...
    axios
      .get('/api/backend/admin/tenent_users/' + user_id + '?tenant=' + tenant)
      .then((response) => {
              ...
              confirmButton.addEventListener('click', async () => {
                ...
                try {
                  const response = await axios.post(
                    `/api/backend/admin/tenent_users/${user_id}/update`,
                    data
                  )
  ...
  function getData(page, data) {
    ...
    var api = '/api/backend/admin/tenent_user' // Base API URL
  ...
  $('#delUserBtn').on('click', function () {
    ...
    if (selectedIds.length > 0) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          var data = { ids: selectedIds, _method: 'DELETE', tenant: tenant }
          try {
            const response = await axios.post(
              '/api/backend/admin/tenent_users',
              data
            )
